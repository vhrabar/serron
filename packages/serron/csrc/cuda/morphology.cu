...
/**
 * Grayscale dilation kernel, one thread per output element.
 *
 * Computes out(n,c,i,j) = max_{(di,dj)} [ in(n,c,i+di-aH,j+dj-aW) + se(c,di,dj) ],
 * the dual of erode_kernel: dilate(f) == -erode(-f) under the same structuring
 * element. Out-of-image samples are resolved by @p border (kConstant contributes
 * the -inf neutral element).
 *
 * @tparam scalar_t              Element type of @p input / @p kernel / @p output; the reduction accumulates in
 * at::acc_type<scalar_t>.
 * @param input                  Input image, contiguous (N, C, H, W).
 ...
 */
template <typename scalar_t>
__global__ void dilate_kernel(const scalar_t* __restrict__ input, const scalar_t* __restrict__ kernel,
                              scalar_t* __restrict__ output, const int64_t N, const int64_t C, const int64_t H,
                              const int64_t W, const int64_t kH, const int64_t kW, const int64_t kernel_channel_stride,
                              const int border) {
    using acc_t = at::acc_type<scalar_t, true>;
    ...
    // Neutral element for a max-reduction;
    const auto kNegInf = static_cast<acc_t>(-INFINITY);
    acc_t acc = kNegInf;
...
at::Tensor dilate(const at::Tensor& input, const at::Tensor& kernel, int64_t border) {
    TORCH_CHECK(input.is_cuda(), "serron::dilate: input must be a CUDA tensor");
    TORCH_CHECK(kernel.is_cuda(), "serron::dilate: kernel must be a CUDA tensor");
    TORCH_CHECK(input.dim() == 4, "serron::dilate: input must be 4-D (N, C, H, W), got ", input.dim(), "-D");
    TORCH_CHECK(kernel.dim() == 2 || kernel.dim() == 3,
                "serron::dilate: kernel must be 2-D (kH, kW) or 3-D (C, kH, kW), got ", kernel.dim(), "-D");
    TORCH_CHECK(input.scalar_type() == kernel.scalar_type(), "serron::dilate: input and kernel must share a dtype");
    TORCH_CHECK(border >= 0 && border <= 2, "serron::dilate: invalid border mode ", border);
    ...
    int64_t kernel_channel_stride = 0;
    if (kernel_c.dim() == 3) {
        TORCH_CHECK(kernel_c.size(0) == C, "serron::dilate: kernel channel dim (", kernel_c.size(0),
                    ") must match input channels (", C, ")");
        kH = kernel_c.size(1);
        kW = kernel_c.size(2);
        kernel_channel_stride = kH * kW;
    ...
    TORCH_CHECK(kH > 0 && kW > 0, "serron::dilate: kernel spatial dims must be positive");
    at::Tensor output = at::empty_like(input_c);
    const int64_t total = output.numel();
    if (total == 0)
        return output;
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16, input_c.scalar_type(), "serron_dilate", [&] {
            dilate_kernel<scalar_t><<<blocks, THREADS, 0, stream>>>(
                input_c.data_ptr<scalar_t>(), kernel_c.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(), N, C, H, W,
                kH, kW, kernel_channel_stride, static_cast<int>(border));
        });
    C10_CUDA_KERNEL_LAUNCH_CHECK();
