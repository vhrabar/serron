#ifndef SERRON_CPU_BOUNDARIES_H
#define SERRON_CPU_BOUNDARIES_H

#include <cstdint>

/**
 * Boundary handling for out-of-image neighbourhood samples (CPU backend).
 *
 * Must stay bit-for-bit identical to the CUDA @c resolve_coord so the two backends resolve borders the same way.
 */
enum BorderMode : int {
 ...
 * outside the image.
 */
inline bool resolve_coord(int64_t& coord, const int64_t size, const BorderMode border) {
